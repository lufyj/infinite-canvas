...
export async function hydrateNodeGenerationContext(context: NodeGenerationContext, options?: { preservePublicUrls?: boolean }) {
    ...
    return {
        ...
        referenceImages: await Promise.all(context.referenceImages.map(async (image) => {
            ...
            if (options?.preservePublicUrls && publicUrl) return { ...image, dataUrl: publicUrl };
