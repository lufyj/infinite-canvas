...
import { FMGO_PLUGIN_MODELS } from "@/lib/fmgo-models";
import { fetchImageModels } from "@/services/api/image";
import { guessCapability, type ModelChannel } from "@/stores/use-config-store";

const FIXED_FMGO_MODELS = new Set(FMGO_PLUGIN_MODELS);
...
export function ModelSelectModal({ open, channel, selectedNames, onConfirm, onClose }: { open: boolean; channel: ModelChannel | null; selectedNames: string[]; onConfirm: (names: string[]) => void; onClose: () => void }) {
    ...
    const addManual = () => {
        ...
        if (!name || FIXED_FMGO_MODELS.has(name)) return;
    ...
    const fetchModels = async () => {
        ...
        try {
            ...
            const editableModels = models.filter((name) => {
                const capability = guessCapability(name);
                return !FIXED_FMGO_MODELS.has(name) && (capability === "text" || capability === "audio");
            });
            setFetched(editableModels);
            setActiveTab("new");
            message.success(t("config.modelSelect.fetched", { count: editableModels.length }));
        } catch (error) {
