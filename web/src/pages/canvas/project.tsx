...
function InfiniteCanvasPage() {
        ...
        async (nodeId: string, mode: CanvasNodeGenerationMode, prompt: string) => {
            ...
            if (sourceNode && builtinPanel?.writeBackToSelf && builtinPanel.mode === "image") {
                ...
                try {
                    ...
                    const context = await hydrateNodeGenerationContext(buildNodeGenerationContext(nodeId, nodesRef.current, connectionsRef.current, fullPrompt), { preservePublicUrls: generationConfig.apiFormat === "fmgo" });
            ...
            const generationContext = await hydrateNodeGenerationContext(
                ...
                { preservePublicUrls: generationConfig.apiFormat === "fmgo" },
            );
                ...
                if (mode === "video") {
                    ...
                    try {
                        const video = await storeGeneratedVideo(
                            await requestVideoGeneration(generationConfig, effectivePrompt, generationContext.referenceImages, { signal: controller.signal, referenceVideos: generationContext.referenceVideos, referenceAudios: generationContext.referenceAudios }),
                        );
    ...
    const handleRetryNode = useCallback(
        async (node: CanvasNodeData, imageId?: string) => {
            ...
            const context = hasSavedImageMetadata ? null : await hydrateNodeGenerationContext(buildNodeGenerationContext(sourceNode.id, nodesRef.current, connectionsRef.current, sourceNode.metadata?.prompt || node.metadata?.prompt || ""), { preservePublicUrls: generationConfig.apiFormat === "fmgo" });
