...
+ [调整] 视频生成任务超时时间延长为 1 小时，FMGO 图片生成任务超时时间延长为 30 分钟。

+ [修复] 画布提示词输入框粘贴外部文案时仅保留纯文本，避免网页字体、颜色等富文本样式一并带入。
+ [优化] Docker 镜像工作流改为发布到当前仓库所有者的 GHCR，新增必须显式指定自有镜像的生产 Compose，并补全从镜像发布到服务器部署、升级与回滚的文档。
+ [调整] 默认 FMGO 渠道的文本、音频、图片和视频模型统一改由 `/v1/models` 动态加载，渠道下架模型不再展示或接受新任务，同时保留文本/音频分类与调用脚本配置。
+ [修复] FMGO 动态目录保留逻辑模型对应的实际请求型号，图片分辨率与视频分辨率/时长只展示渠道已开放的组合，并按文档修正 `feimiao-v2-mini` 的固定参数配对。
+ [修复] 修正 FMGO 模型能力常量的初始化顺序，避免应用启动时报 `Cannot access before initialization` 并显示空白页。
+ [修复] FMGO 视频参考图遇到跨域图片时回退使用公网图片 URL，避免在提交任务前因浏览器 `Failed to fetch` 中断。
+ [修复] FMGO `/v1/chat/completions` 图片引用有公网 URL 时直接提交，避免无必要的跨域 Base64 转换。
+ [修复] 画布 FMGO 生成上下文识别存放在 `dataUrl` 字段中的公网图片 URL，避免 hydration 阶段提前触发跨域请求。
+ [新增] FMGO 按更新文档支持 Grok 与 feimiao-v2.5 视频模型。
+ [新增] FMGO 同步视频插件完整模型目录，补充 Sora 2、Veo 3.1 Fast、SS、Minimax、MD、K2 和飞喵变体，并按插件分组展示。
+ [调整] 渠道配置固定为唯一的默认 FMGO 渠道，导入和本地持久化会移除其他渠道，文档外模型不再允许选择或调用。
+ [调整] FMGO 模型选择改为插件逻辑模型，画布节点按当前模型展示参数，并按分辨率和时长解析实际请求模型名。
+ [优化] FMGO 模型选择器按插件系列分组展示，画布、工作台和默认模型设置使用一致分类。

+ [新增] 按 FMGO 接口规范接入图片生成与编辑，支持异步任务轮询、GPT Image 2 系列及 Gemini 图片模型。
+ [新增] 按 FMGO 接口规范接入视频生成，支持 Sora、Veo、Feimiao、Omni 及图片参考任务轮询。
