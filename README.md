...
生产服务器推荐先通过 GitHub Actions 发布到你自己的 GHCR，再使用 `docker-compose.prod.yml` 拉取运行，不在服务器编译前端。完整的仓库配置、镜像发布、GHCR 权限、服务器部署、HTTPS、升级和回滚流程见 [自行发布 Docker 镜像并部署](docs/content/docs/overview/docker.zh-CN.mdx)。
