...
- 接入或限制渠道时，只调整用户明确指定的渠道能力；不得连带删除文本推理、音频、自定义模型或调用脚本等现有功能，除非用户明确要求。
- 渠道配置固定为唯一的默认 FMGO 渠道；不得新增、删除或切换其他协议渠道，配置导入和本地持久化也必须收敛为该渠道。
- FMGO 模型是否可用必须以默认渠道 `/v1/models` 最近一次成功返回的目录为准，渠道未返回的模型不得展示或发起新任务；插件配置只负责图片/视频模型的能力、分组、可选参数和请求模型名转换，目录刷新失败时保留上次成功结果。
- FMGO 接入必须以用户提供的 `D:\\plugin\\api.md` 为唯一协议来源；不得依据其他插件实现猜测模型名、接口路径或请求字段，文档外模型必须显式拒绝。
